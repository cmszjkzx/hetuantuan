...
// +----------------------------------------------------------------------
// | Author: hetuantuan@chinamobilesz.com
// +----------------------------------------------------------------------
...
define('SAPP_NAME', '移动和团团V1.0');
define('CORE_VERSION', 20161012);
...
$pdo = $_CMS['pdo'] = null;
$heconfigfile =  WEB_ROOT."/config/config.php";
$HETUANTUAN_CONFIG=array();
if(is_file($heconfigfile))
{
	require $heconfigfile;
}
$heconfig=$HETUANTUAN_CONFIG;
if(empty($heconfig['db']['host']))
{
	$heconfig['db']['host'] = '';
}
if(empty($heconfig['db']['username']))
{
	$heconfig['db']['username'] = '';
}
if(empty($heconfig['db']['password']))
{
	$heconfig['db']['password'] = '';	
}
if(empty($heconfig['db']['port']))
{
	$heconfig['db']['port'] = '';
}
if(empty($heconfig['db']['database']))
{
	$heconfig['db']['database'] = '';
}
$heconfig['db']['charset'] = 'utf8';
$_CMS['config'] = $heconfig;
...
class PdoUtil {
	...
	public function excute($sql, $stuff = 'hetuantuan_') {
		if(!isset($sql) || empty($sql)) return;

		$sql = str_replace("\r", "\n", str_replace(' ' . $stuff, ' hetuantuan_', $sql));
		$sql = str_replace("\r", "\n", str_replace(' `' . $stuff, ' `hetuantuan_' , $sql));
		$ret = array();
		$num = 0;
		foreach(explode(";\n", trim($sql)) as $query) {
...
//2016-12-2-yanru-begin
function express_order_upload($file, $type = 'file') {
    if(empty($file))
    {
        return error(-1, '没有上传内容');
    }
    ...
    if($type=='other')
    {
        $extentions=array('gif', 'jpg', 'jpeg', 'png','mp3','mp4','ogv', 'webm', 'doc');
    }
    if(!in_array(strtolower($extention), $extentions))
    {
        return error(-1, '不允许上传此类文件');
...
//2017-02-15-yanru-begin
function group_upload($file, $type = 'image') {
    if(empty($file))
    {
        return error(-1, '没有上传内容');
    }
    ...
    if(!file_move($file['tmp_name'], $filename))
    {
        return error(-1, '保存上传文件失败'.$filename.'--'.$file['tmp_name']);
    }
    $result['success'] = true;
    return $result;
}
