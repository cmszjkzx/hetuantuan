...
if($_CMS[WEB_SESSION_ACCOUNT]['is_admin']){
    ...
    $needsend_count = mysqld_selectcolumn("SELECT count(ordergoods.orderid) FROM " . table('shop_order_goods') . " ordergoods LEFT JOIN ".table('shop_goods')." shopgoods ON ordergoods.goodsid=shopgoods.id LEFT JOIN ". table('shop_order') . " shoporder ON shoporder.id=ordergoods.orderid WHERE shopgoods.band = :band AND (shoporder.status = 2 OR (shoporder.status = 3 AND ordergoods.status = 11)) "."  ", array(':band'=>$_CMS[WEB_SESSION_ACCOUNT]['groupName']));

    $needsend__price = mysqld_selectcolumn("SELECT sum(ordergoods.price) FROM " . table('shop_order_goods') . " ordergoods LEFT JOIN ".table('shop_goods')." shopgoods ON ordergoods.goodsid=shopgoods.id LEFT JOIN ". table('shop_order') . " shoporder ON shoporder.id=ordergoods.orderid WHERE shopgoods.band = :band AND (shoporders.status = 2 OR (shoporders.status = 3 AND ordergoods.status = 11)) "."  ", array(':band'=>$_CMS[WEB_SESSION_ACCOUNT]['groupName']));
    $needget_count = mysqld_selectcolumn("SELECT count(ordergoods.orderid) FROM " . table('shop_order_goods') . " ordergoods LEFT JOIN ".table('shop_goods')." shopgoods ON ordergoods.goodsid=shopgoods.id LEFT JOIN ". table('shop_order') . " shoporder ON shoporder.id=ordergoods.orderid WHERE shopgoods.band = :band AND shoporder.status = 3 AND ordergoods.status = 12 "."  ", array(':band'=>$_CMS[WEB_SESSION_ACCOUNT]['groupName']));
    $needget__price = mysqld_selectcolumn("SELECT sum(ordergoods.price) FROM " . table('shop_order_goods') . " ordergoods LEFT JOIN ".table('shop_goods')." shopgoods ON ordergoods.goodsid=shopgoods.id LEFT JOIN ". table('shop_order') . " shoporder ON shoporder.id=ordergoods.orderid WHERE shopgoods.band = :band AND shoporders.status = 3 AND ordergoods.status = 12 "."  ", array(':band'=>$_CMS[WEB_SESSION_ACCOUNT]['groupName']));
