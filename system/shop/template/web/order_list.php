...
<form action="" method="get">
    ...
    <table  class="table" style="width:95%;" align="center">
        ...
        <tr>
            ...
            <?php } else{?>
                <input type="hidden" id="bandmanage" name="bandmanage" value="<?php  echo empty($_GP['bandmanage'])?$_CMS[WEB_SESSION_ACCOUNT]['groupName']:$_GP['bandmanage']; ?>"/>
            <?php } ?>
        </tr>
        <tr>
            <td ></td>
            <td colspan="3">
                ...
                <button type="submit" name="report" value="report" class="btn btn-warning">导出订单</button>
            </td>
        </tr>
        </tbody>
    </table>
...
<form action="" method="post" name="getexcelform" enctype="multipart/form-data" class="form-horizontal" role="form">
    <table  class="table" style="width:95%;" align="center">
        <tbody>
        <tr>
            <div class="fileupload fileupload-new" data-provides="fileupload">
                <input name="expressorder" id="expressorder" type="file" />
            </div>
            <button type="submit" name="getexcel" value="getexcel" class="btn btn-success">导入运单</button>
        </tr>
        </tbody>
    </table>
</form>
...
<table class="table table-striped table-bordered table-hover">
    ...
    <?php  if(is_array($list)) { foreach($list as $item) { ?>
        <tr>
            ...
            <td><?php  if(empty($item['optionsprice'])) { echo $item['price']; } else { echo $item['optionsprice']; }?> 元 <?php  if($item['hasbonus']>0) { ?><span class="label label-success">惠<?php echo $item['bonusprice'];?></span><?php  }?></td>
            <td>
        <?php if(empty($_CMS[WEB_SESSION_ACCOUNT]['is_admin']) || !empty($_GP['bandmanage'])){ ?>
            ...
            <?php  if(($order['status'] == 2 || $order['status'] == 3) && ($item['optionstatus'] == 11 || $item['optionstatus'] == 0)) { ?><span class="label label-warning">待发货</span><?php  } ?>
            ...
            <td><a class="btn btn-xs btn-info"  href="<?php if(empty($_GP['bandmanage'])){echo web_url('order', array('op' => 'detail', 'id' => $item['id']));}else{echo web_url('order', array('op' => 'detail', 'id' => $item['id'], 'bandmanage'=>$_GP['bandmanage']));}?>"><i class="icon-edit"></i>查看详情</a>
                <?php  if($hasaddon11) { ?>
