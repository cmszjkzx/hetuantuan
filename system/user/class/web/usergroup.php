...
if ($operation == 'usergroup') {
    ...
    $bands = mysqld_selectall("select * from ( select distinct groupName from ( select distinct if(band!='', band, '其他') as groupName from " .table('shop_goods'). " union select distinct groupName from ".table('user_group')." ) hetuantuan_test_group union all select distinct groupName from hetuantuan_user_group ) hetuantuan_temp group by groupName having count(groupName)=1");
    if (checksubmit('submit')) {
        if(empty($_GP['groupName']) && empty($_GP['band']))
        {
            message('用户组名称不能空');
            exit;
        }
        if(!empty($_GP['groupName'])){
            $getGroupName = $_GP['groupName'];
        }else{
            $getGroupName = $_GP['band'];
        }
        if(empty($_GP['id']))
        {
            //$data= array('groupName'=> $_GP['groupName'],'createtime'=>time());
            $data= array('groupName'=> $getGroupName,'createtime'=>time());
        ...
        }else
        {
            ...
            mysqld_update('user_group',array('groupName'=> $getGroupName),array('id'=> $_GP['id']));
